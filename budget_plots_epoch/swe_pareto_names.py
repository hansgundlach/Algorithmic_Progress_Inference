"""
SWE-bench Verified Pareto Frontier with Model Names (ICML Style)
...
def main():
    ...
    parser.add_argument("--out", default="../figures/swe_pareto_with_names.png")
    ...
    # Figure size — shared with swe_pareto_frontier.py
    ...
    # =========================================================================
    # AXIS LIMITS — shared with swe_pareto_frontier.py
    # =========================================================================
    XMIN = PAIR_XMIN
    XMAX = PAIR_XMAX
    YMIN = PAIR_YMIN
    ...
    # Drop rows with missing data or zero/negative scores
    df = df.dropna(subset=[price_col, "error_rate", benchmark_col])
    df = df[df[price_col] > 0].copy()
    df = df[df[benchmark_col] > 0].copy()
        ...
        for label_info in labels_data:
            ...
            if needs_arrow:
                ax.annotate(
                    name,
                    xy=(x_data, y_data),
                    xytext=(label_data_pos[0], label_y_data),
                    textcoords="data",
                    fontsize=MODEL_NAME_FONTSIZE,
                    fontweight=LABEL_FONTWEIGHT,
                    color=color,
    ...
    ax.set_title(
        "SWE-bench Verified Pareto Frontier\nwith Model Names",
    ...
    # Fixed margins — must match swe_pareto_frontier.py for identical axes boxes
