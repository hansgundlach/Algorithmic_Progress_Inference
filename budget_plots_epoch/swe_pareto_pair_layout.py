"""
Shared layout for SWE-bench Verified Pareto figures that are placed side-by-side in LaTeX.
...
# whitespace above the axes but the x/y box matches the companion plot.
...
def _print_usage() -> None:
    print(
        "swe_pareto_pair_layout.py — library only (layout constants + helpers).\n"
        ...
        "  python swe_pareto_names.py\n"
        "      -> ../figures/swe_pareto_with_names.png (+ .pdf)\n"
        "  python swe_pareto_frontier.py\n"
        "      -> ../figures/swe_pareto_frontier.png (+ .pdf)\n\n"
        "Optional: python swe_pareto_pair_layout.py --demo\n"
        "      -> _swe_pair_layout_demo.png here in budget_plots_epoch/ only (not figures/)\n"
    )
...
def _demo() -> None:
    ...
    ax.plot([0, 1], [0, 1])
    ax.set_title("SWE pair layout demo (shared axes box)\nsecond line")
    apply_pair_layout(fig)
    out = Path(__file__).resolve().parent / "_swe_pair_layout_demo.png"
...
if __name__ == "__main__":
    ...
    p = argparse.ArgumentParser(description="SWE-bench Pareto pair layout helpers (library).")
    p.add_argument(
        "--demo",
        action="store_true",
        help="Draw a tiny test figure and save _swe_pair_layout_demo.png next to this file",
    )
    args = p.parse_args()
    if args.demo:
        _demo()
