"""
SWE-bench Verified Pareto Frontier Analysis (Epoch AI Style)

Publication-quality visualization showing:
- Frontier models (best at any price) with trend line
- Price-controlled trend lines ($100 and fitted price)
...
def main():
    ...
    parser.add_argument("--out", default="../figures/swe_pareto_frontier.png")
    ...
    PRICE_REFERENCE = 100.0  # SWE benchmark costs are higher than GPQA/AIME
    ...
    log_price_ref = np.log10(PRICE_REFERENCE)
    pred_price_ref_logit = (
        model_price_controlled.intercept_
        + model_price_controlled.coef_[0] * years_range.values
        + model_price_controlled.coef_[1] * log_price_ref
    )
    ...
    # Reference price trend line (teal) - solid then dashed
    ax.plot(
        dates_range[: extrapolation_start_idx + 1],
        pred_price_ref_logit[: extrapolation_start_idx + 1],
        "-",
    ...
    ax.plot(
        dates_range[extrapolation_start_idx:],
        pred_price_ref_logit[extrapolation_start_idx:],
        "--",
        color=COLOR_PRICE_LINES,
        linewidth=2,
        alpha=0.6,
    ...
    # Reference price line label
    end_score_ref = pred_price_ref_logit[end_idx]
    price_ref_label = f"${PRICE_REFERENCE:.0f}"
    ax.annotate(
        price_ref_label,
        xy=(end_date, end_score_ref),
        xytext=(8, 0),
    ...
    anno_score_price = pred_price_ref_logit[anno_idx]
    ax.annotate(
    ...
    ax.set_ylabel("SWE-bench Score (logits)", fontsize=FONT_SIZE_LABEL)
    ...
    # Print summary
    print(f"\nSWE-bench Verified Pareto Analysis")
