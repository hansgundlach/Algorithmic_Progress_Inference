"""
AIME Budget Step Plot (Epoch AI Style)
...
def load_data(path: str) -> pd.DataFrame:
    ...
    df["Score"] = df["oneshot_AIME"].astype(str).str.replace("%", "").astype(float)
    df["Price"] = (
...
def main():
    ...
    parser.add_argument("--out", default="../figures/aime_budget_epoch.png")
    ...
    ax.set_ylabel(
        "AIME Accuracy (logit)", fontsize=18, fontweight="bold", color=TEXT_COLOR
    )
    ax.set_title(
    ...
    for item in line_data_sorted:
        # Adjust vertical offset for labels to avoid overlap
        if item["label"] == "$0.10":
            vertical_offset = -12
        else:
            vertical_offset = 0
