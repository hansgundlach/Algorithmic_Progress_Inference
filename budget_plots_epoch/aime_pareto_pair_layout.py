"""
Shared layout for AIME Pareto figures that are placed side-by-side in LaTeX.
...
def _print_usage() -> None:
    print(
        "aime_pareto_pair_layout.py — library only (layout constants + helpers).\n"
        ...
        "  python aime_pareto_names.py\n"
        "      -> ../figures/aime_pareto_with_names.png (+ .pdf)\n"
        "  python aime_pareto_frontier.py\n"
        "      -> ../figures/aime_pareto_frontier.png (+ .pdf)\n\n"
        "Optional: python aime_pareto_pair_layout.py --demo\n"
        "      -> _aime_pair_layout_demo.png here in budget_plots_epoch/ only (not figures/)\n"
    )
...
def _demo() -> None:
    ...
    ax.plot([0, 1], [0, 1])
    ax.set_title("AIME pair layout demo (shared axes box)\nsecond line")
    apply_pair_layout(fig)
    out = Path(__file__).resolve().parent / "_aime_pair_layout_demo.png"
...
if __name__ == "__main__":
    ...
    p = argparse.ArgumentParser(description="AIME Pareto pair layout helpers (library).")
    p.add_argument(
        "--demo",
        action="store_true",
        help="Draw a tiny test figure and save _aime_pair_layout_demo.png next to this file",
    )
    args = p.parse_args()
    if args.demo:
        _demo()
