...
def plot_benchmark_price_vs_time(
    ...
    if "Release Date" in df_work.columns:
        df_work["Release Date"] = pd.to_datetime(
            df_work["Release Date"], errors="coerce"
        )
    else:
    ...
    if not include_reasoning_models and "Reasoning_TF" in df_sub.columns:
        df_sub = df_sub[
            (df_sub["Reasoning_TF"] != True) | (df_sub["Reasoning_TF"].isna())
        ]
    ...
    if exclude_dominated:
        ...
        for i, row in df_sub.iterrows():
            ...
            for j in non_dominated:
                ...
                if (
                    prev_row[benchmark_col] >= row[benchmark_col]
    ...
    if fit_overall_trend and len(df_sub) >= 3:
        ...
        if use_quantile_regression:
            ...
            overall_model = QuantileRegressor(
        ...
        overall_slope = (
            overall_model.coef_[0]
            if hasattr(overall_model, "coef_")
            else overall_model.coef_
        )
        ...
        if confidence_interval and len(df_sub) > 2 and not use_quantile_regression:
            ...
            annual_factor_lower = (
                1 / np.exp(ci_upper * 365)
                if overall_factor_decrease_per_year
                else np.exp(ci_lower * 365)
            )
            annual_factor_upper = (
                1 / np.exp(ci_lower * 365)
                if overall_factor_decrease_per_year
                else np.exp(ci_upper * 365)
            )
            overall_ci_text = (
        ...
        regression_type = (
            f"Q{int(quantile*100)} Quantile" if use_quantile_regression else "OLS"
        )
    ...
    if record_price_trend and len(df_sub) >= 3:
        ...
        if len(df_regression) >= 3:
            ...
            if use_quantile_regression:
                # Use quantile regression
                record_model = QuantileRegressor(
                    quantile=quantile, alpha=0, solver="highs"
                ).fit(X_record, y_log_record)
                # For quantile regression, we can't calculate R² in the same way
                # Instead we can compute pseudo R² based on quantile loss
                y_pred_record = record_model.predict(X_record)
                # Calculate quantile loss for baseline (median of y) and model
                baseline_pred_record = np.full_like(
                    y_log_record, np.quantile(y_log_record, quantile)
                )
                ...
                baseline_loss_record = quantile_loss(
                    y_log_record, baseline_pred_record, quantile
                )
                model_loss_record = quantile_loss(y_log_record, y_pred_record, quantile)
                record_r2 = (
                    1 - (model_loss_record / baseline_loss_record)
                    if baseline_loss_record > 0
                    else 0
                )
            else:
                # Use standard linear regression
            ...
            # Calculate annual change
            record_slope = (
                record_model.coef_[0]
                if hasattr(record_model, "coef_")
                else record_model.coef_
            )
    ...
    default_title_text = (
        f'{price_col.replace("_", " ")} vs Release Date{filter_text}{benchmark_range}'
    )
    ...
    cbar = plt.colorbar(
        scatter, shrink=colorbar_shrink, aspect=colorbar_aspect, pad=colorbar_pad
    )
    ...
    plt.grid(
        ...
        linewidth=major_grid_linewidth,
    ...
    plt.grid(
        ...
        linewidth=minor_grid_linewidth,
    ...
    print(
        f"Price range: ${df_sub[price_col].min():.2f} - ${df_sub[price_col].max():.2f}"
    )
    print(
        f"{benchmark_name} range: {df_sub[benchmark_col].min():.1f} - {df_sub[benchmark_col].max():.1f}"
    )
    print(
        f"Date range: {df_sub['Release Date'].min().strftime('%Y-%m-%d')} to {df_sub['Release Date'].max().strftime('%Y-%m-%d')}"
    )
    ...
    if overall_stats:
        ...
        if overall_stats["factor_decrease_per_year"]:
            print(
                f"Price becomes {overall_stats['factor_decrease_per_year']:.1f}x cheaper each year"
            )
        else:
            print(
                f"Price changes by factor of {overall_stats['factor_change_per_year']:.1f}x each year"
            )
    ...
    if record_stats:
        ...
        if record_stats["factor_decrease_per_year"]:
            print(
                f"Price becomes {record_stats['factor_decrease_per_year']:.1f}x cheaper each year"
            )
        else:
            print(
                f"Price changes by factor of {record_stats['factor_change_per_year']:.1f}x each year"
            )
        metric_name = "Pseudo-R²" if use_quantile_regression else "R²"
